#pragma once

namespace glanguage
{
	//lable
	inline constexpr wchar_t lable[] = L"按下 Ins（Insert）显示/隐藏菜单\n按下 Del（Delete）退出";

	//visual
	inline constexpr wchar_t visual[] = L"视觉";
	inline constexpr wchar_t player[] = L"玩家";
	inline constexpr wchar_t item[] = L"物品";
	inline constexpr wchar_t misc[] = L"杂项";
	inline constexpr wchar_t entity[] = L"实体";
	inline constexpr wchar_t interact[] = L"互动";
	inline constexpr wchar_t enable[] = L"启用";
	inline constexpr wchar_t box[] = L"方框";
	inline constexpr wchar_t name[] = L"名称";
	inline constexpr wchar_t distance[] = L"距离";
	inline constexpr wchar_t line[] = L"连线";
	inline constexpr wchar_t extent[] = L"范围";
	inline constexpr wchar_t esp_distance[] = L"透视距离：";
	inline constexpr wchar_t meter[] = L"米";
	inline constexpr wchar_t draw_mesh[] = L"绘制周围网格体";
	inline constexpr wchar_t draw_mesh_distance[] = L"网格体距离：";
	inline constexpr wchar_t fov[] = L"视野：";
	inline constexpr wchar_t disable_post[] = L"禁用后处理";
	inline constexpr wchar_t third_person[] = L"第三人称";
	inline constexpr wchar_t camera_len[] = L"摇臂长度：";

	//player
	inline constexpr wchar_t inf_stamina[] = L"无限耐力";
	inline constexpr wchar_t inf_sanity[] = L"无限San值";
	inline constexpr wchar_t auto_balance[] = L"自动平衡";
	inline constexpr wchar_t inf_jump[] = L"无限跳跃";
	inline constexpr wchar_t walk_speed[] = L"行走速度：";
	inline constexpr wchar_t run_speed[] = L"跑步速度：";
	inline constexpr wchar_t crouch_speed[] = L"蹲下速度：";
}